...
- **AI**: Google Gemini (`gemini-2.5-flash` via `v1beta` API for analysis/cache/grounding/questions, `gemini-2.0-flash` for voice/dialogue, `MEDIA_RESOLUTION_LOW` on generateContent, 1M token context)
...
- Cache creation via standard SDK `caches.create()` (no mediaResolution — that only works on generateContent). 2.5-flash has 1M token limit; videos exceeding this skip caching and use direct file references.
...
1. **Cache available** → Uses `gemini-2.5-flash` with cached content (preferred)
